/*
  Bruce A. Maxwell, Matej Zecic
  January 2025

  ***NOTE***: Modified min, max values for fixed clipping normalization - Matej Zecic
  ...
  The result image is a greyscale image with values in the range of
...
public:
  ...
  DA2Network( const char *network_path ) {
    ...
    std::strncpy( input_names_, "pixel_values", 255 ); // default values for the network model_fp16.onnx
  ...
  int run_network( cv::Mat &dst, const cv::Size &output_size ) {
    ...
    // **Modified Section: Fixed Clipping Normalization**
    float fixed_min = 0.0;
    float fixed_max = 10.0;
    ...
    for(int i=0,k=0;i<out_height_;i++) {
      ...
      for(int j=0;j<out_width_;j++, k++) {
        float value = 255 * (tensorData[k] - fixed_min) / (fixed_max - fixed_min);
        ptr[j] = value > 255.0 ? (unsigned char)255 : (value < 0.0 ? 0 : static_cast<unsigned char>(value));
      }
    }
 ...
 private:
  ...
  std::array<int64_t, 4> input_shape_{1, 3, 0, 0 }; // batch, channel, height, width: 3-channel color image
};
